...
import {
  ...
  IndianRupee, ShieldCheck, Tag, Edit2,
} from 'lucide-react';
...
const Plans = () => {
  ...
  const handleSavePlan = async (e) => {
    ...
    try {
      if (editingPlanId) {
        await api.put(`/plans/${editingPlanId}`, newPlan);
  ...
  return (
    <PageTransition>
      <div className="space-y-6 lg:space-y-8 max-w-screen-2xl mx-auto pb-safe-area">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => {
                          ...
                          setNewPlan({ name: plan.name, duration_days: plan.duration_days, price: plan.price });
                          setShowModal(true);
                        }}
                        className="w-9 h-9 text-slate-600 hover:text-earth-clay hover:bg-earth-clay/[0.08] rounded-xl transition-all border border-transparent hover:border-earth-clay/20 flex items-center justify-center"
        ...
        <Modal
          isOpen={showModal}
          onClose={() => { setShowModal(false); setEditingPlanId(null); setNewPlan({ name: '', duration_days: '', price: '' }); }}
          title={editingPlanId ? "Edit Membership Plan" : "Create Membership Plan"}
          subtitle={editingPlanId ? "Update plan details" : "This plan will be immediately available for enrollment"}
          maxWidth="max-w-md"
        >
