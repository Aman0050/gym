          ...
          <div className="page-header shadow-xl">
            <div className="page-header-meta">
              <div className="flex items-center gap-3">
                ...
                <span className="label-text">Branch Overview</span>
              </div>
              <h1 className="page-title text-ivory">
                Operational{' '}
                <span className="text-earth-clay italic">Hub</span>
              </h1>
            </div>
          ...
          <div className="lg:col-span-8 space-y-6 lg:space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-stretch gap-5">
                ...
                {stats.map((stat, idx) => (
                  <FadeIn key={stat.label} direction="up" delay={idx * 0.06} duration={0.4}>
                    <Card
                      ...
                      className={`p-6 flex flex-col justify-between h-36 lg:h-40 overflow-hidden relative group ${!stat.priority ? 'hidden xl:flex' : ''}`}
                    >
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-white/[0.05] border border-white/[0.07] text-earth-clay relative z-10">
                        <stat.icon size={16} />
                      </div>
                      <div className="relative z-10">
                        <p className="label-text !text-[8px] !text-slate-500 mb-1.5">{stat.label}</p>
                        <h3 className="text-2xl lg:text-3xl font-black text-ivory leading-none tracking-tight flex items-baseline gap-1">
                          {stat.isCurrency && (
              ...
              <button
                ...
                className="sm:w-16 flex flex-col items-center justify-center gap-2 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] rounded-3xl transition-all group"
              >
                <TrendingUp size={18} className={`${showAnalytics ? 'text-earth-clay' : 'text-slate-600'} group-hover:scale-110 transition-transform`} />
                <span className="text-[8px] font-black uppercase tracking-tighter text-slate-600 vertical-text hidden sm:block">
                  {showAnalytics ? 'Collapse' : 'Analytics'}
                </span>
              </button>
            </div>

            {/* Revenue Chart (Progressive Disclosure) */}
            <AnimatePresence>
              {showAnalytics && (
                <motion.div
                ...
                {expiringSoon.length === 0 ? (
                  ...
                  expiringSoon.map((m, i) => (
                    <div
                      ...
                      className={`
                        bg-white/[0.03] border p-4 rounded-2xl flex items-center justify-between group cursor-pointer transition-all duration-200
                        ${new Date(m.valid_until) < new Date() ? 'border-red-500/20 bg-red-500/5' : 'border-white/[0.06] hover:border-earth-clay/25'}
                      `}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 border ${
                          new Date(m.valid_until) < new Date() ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-terracotta/10 border-terracotta/15 text-terracotta'
                        }`}>
                          <User size={13} />
                        </div>
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <p className="text-[9px] text-slate-500 font-semibold uppercase">
                              {new Date(m.valid_until) < new Date() ? 'Expired' : 'Due'} {new Date(m.valid_until).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                            </p>
                            <PriorityBadge 
                              level={
                                new Date(m.valid_until) < new Date() ? 'CRITICAL' : 
                                (new Date(m.valid_until) - new Date()) / (1000 * 60 * 60 * 24) < 1 ? 'URGENT' : 'WARNING'
                              }
                            />
                          </div>
                        </div>
                      </div>
                      <button className="text-[10px] font-black text-earth-clay opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest hidden sm:block">
                        Resolve Now
                      </button>
