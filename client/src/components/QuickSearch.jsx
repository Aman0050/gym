...
import { Search, User, CreditCard, Activity, Loader2, ArrowRight, Building2 } from 'lucide-react';
...
const QuickSearch = () => {
  ...
  const handleSelect = (result) => {
    ...
    switch (result.type) {
      ...
      case 'PAYMENT':
        ...
        break;
      case 'GYM':
        navigate('/dashboard'); // SuperAdmin dashboard manages gyms
        break;
      default:
        break;
    }
  ...
  return (
    <div className="relative w-64 lg:w-72 hidden md:block" ref={searchRef}>
      <div className={`
        flex items-center bg-[#0d0d0d]/80 border px-3.5 py-2 rounded-full gap-2.5 group transition-all duration-300
        ${showResults && results.length > 0 ? 'border-earth-clay/40 bg-white/[0.06] rounded-b-none' : 'border-white/[0.08] focus-within:border-earth-clay/35 focus-within:bg-[#080808]'}
      `}>
        {isSearching ? (
          <Loader2 size={13} className="text-earth-clay animate-spin flex-shrink-0" />
        ) : (
          <Search size={13} className="text-slate-500 group-focus-within:text-earth-clay transition-colors flex-shrink-0" />
        )}
        <input
          type="text"
          ...
          className="bg-transparent border-none focus:ring-0 text-[11px] font-medium w-full placeholder:text-slate-400 text-white outline-none"
        />
        <kbd className="text-[8px] text-slate-600 font-mono border border-white/[0.06] rounded px-1.5 py-0.5 hidden lg:block uppercase tracking-wider">⌘K</kbd>
      </div>
