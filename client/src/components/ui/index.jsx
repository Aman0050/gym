...
import { AlertCircle, CheckCircle2, Info, X, ChevronDown, Activity, Search, ArrowLeft } from 'lucide-react';
...
export const EmptyState = memo(({ 
  ...
  <motion.div 
    ...
    className="flex flex-col items-center justify-center text-center p-12 lg:p-20 space-y-6 max-w-[32rem] mx-auto"
  >
...
export const Input = memo(({ label, icon: Icon, action, error, className = '', ...props }) => (
  <div className="space-y-2.5 w-full">
    ...
    <div className="relative">
      ...
      <input
        {...props}
        className={`input-field ${Icon ? 'pl-14' : ''} ${action ? '!pr-12' : ''} ${
          error
...
export const Modal = memo(({ 
  ...
  const widthStyles = {
    ...
    'max-w-3xl': { maxWidth: '768px' },
    'max-w-4xl': { maxWidth: '896px' },
    'max-w-5xl': { maxWidth: '1024px' },
    'max-w-6xl': { maxWidth: '1152px' },
    'max-w-7xl': { maxWidth: '1280px' },
  };
  const resolvedStyle = widthStyles[maxWidth] || (maxWidth.startsWith('max-w-[') ? { maxWidth: maxWidth.slice(6, -1) } : { maxWidth });
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6">
          ...
          <motion.div
            ...
            className="aura-glass-heavy w-full relative overflow-hidden shadow-[0_40px_80px_-20px_rgba(0,0,0,0.6)] max-h-[90vh] flex flex-col"
            style={resolvedStyle}
            ...
            <div className="px-8 py-7 border-b border-white/[0.07] flex items-center justify-between bg-white/[0.02] flex-shrink-0">
              <div className="flex items-center gap-4">
                {onBack && (
                  <button
                    onClick={onBack}
                    className="touch-target p-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-slate-500 hover:text-ivory transition-all border border-white/[0.07] flex-shrink-0 group"
                    aria-label="Back"
                  >
...
export const Table = memo(({ 
  ...
  <div 
    className={`aura-glass-heavy border border-white/[0.08] shadow-[0_8px_30px_rgb(0,0,0,0.12)] bg-gradient-to-b from-white/[0.02] to-transparent overflow-hidden flex flex-col rounded-[1.25rem] relative min-w-0 min-h-0 isolate ${className}`}
...
export const TableRow = memo(({ children, className = '', onClick, isSelected = false }) => {
  return (
    <motion.tr
      ...
      className={`
        group transition-all duration-300 relative border-b border-white/[0.02] last:border-0
        ${onClick ? 'cursor-pointer' : ''}
        ${isSelected ? 'bg-earth-clay/10 border-l-2 border-l-earth-clay' : 'hover:bg-white/[0.04] hover:shadow-[inset_0_1px_0_0_rgba(160,82,45,0.1),inset_0_-1px_0_0_rgba(160,82,45,0.1)]'}
        ${className}
      `}
    >
