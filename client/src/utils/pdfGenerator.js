...
export const generateInvoicePDF = (payment, member) => {
  ...
  doc.text(`Thank you for being part of the ${branchName} community!`, pageWidth / 2, 282, { align: 'center' });

  doc.save(`${branchName.toUpperCase().replace(/\s/g, '_')}_INVOICE_${member.name.replace(/\s/g, '_')}.pdf`);
};
