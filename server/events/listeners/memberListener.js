...
const initMemberListeners = () => {
  eventBus.on(EVENTS.MEMBER_CREATED, async (data) => {
    ...
    try {
      await createNotification({
        ...
        message: `${member.name} has registered at your gym.`
      });
    } catch (error) {
  ...
  eventBus.on('member.inactive', async (data) => {
    ...
    await createNotification({
      ...
      message: `Member ${memberName} hasn't visited in 14 days. Consider reaching out!`
    });
  });
  ...
  eventBus.on('member.reminder_due', async (data) => {
    ...
    await createNotification({
      ...
      message: `${memberName}'s membership expires on ${new Date(expiryDate).toLocaleDateString()}. Follow up for renewal.`
    });
  });
