...
const initSocket = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    if (socket.user.role === 'SUPER_ADMIN') {
      socket.join('platform-admin-room');
      logger.info(`Socket Connected: Super Admin ${socket.user.id} joined platform-admin-room`);
    }
...
const broadcastToSuperAdmins = (eventName, data) => {
  if (io) {
    io.to('platform-admin-room').emit(eventName, data);
    logger.info(`Broadcasted ${eventName} to platform-admin-room`);
  }
};

module.exports = { initSocket, broadcastToGym, disconnectGym, broadcastToSuperAdmins };
